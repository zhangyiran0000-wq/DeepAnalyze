...
(() => {
  ...
  const ui = (value) => {
    ...
    let match = /^Running the (exploration|synthesis|consolidation|evidence_reading|local_synthesis|relation_review|regrouping|evidence_feedback) role within the configured model-call budget\.$/.exec(value);
    if (match) return `正在执行${({exploration:"探索",synthesis:"归纳",consolidation:"合并审查",evidence_reading:"补读证据",local_synthesis:"局部归纳",relation_review:"关系证据审查",regrouping:"主线归并",evidence_feedback:"缺口交回探索"})[match[1]]}，受配置的调用预算限制。`;
  ...
  function renderSnapshot(snapshot) {
    ...
    const metrics=snapshot.metrics||{}; $("map-metrics").textContent=`${metrics.node_count??nodes.length} ${ui("papers")} · ${metrics.edge_count??list(snapshot.edges).length} ${ui("connections")} · ${ui("depth")} ${metrics.depth??0}`;
  ...
  function paperCard(node, group, index) {
    ...
    const top=el("div","card-top");top.append(el("h4","",nodeName(node)));if(node.id===state.snapshot.seed_id)top.append(el("span","seed-badge",ui("SEED"))); if(node.context_role === "background") top.append(el("span","context-badge",ui("Background"))); card.append(top,el("p","card-problem",node.solves||node.problem||"Problem connection has not yet been established."));
